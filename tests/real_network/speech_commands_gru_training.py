"""Speech Commands v0.02 training comparison for torch GRU and QuantGRU QAT.

The model adapts Google Research kws_streaming's GRU topology to the standard
GRU interface: MFCC features, one GRU returning its final state, dropout, and a
dense classifier.
"""
...
class SpeechCommandsGruClassifier(nn.Module):
    """MFCC-GRU-Dropout-Dense keyword classifier."""
...
def _copy_shared_initial_state(source: nn.Module, target: nn.Module) -> float:
    ...
    if source_parameters.keys() != target_parameters.keys():
        raise RuntimeError("model parameter names differ beyond the GRU implementation")
    with torch.no_grad():
...
def _calibrate_quant_gru(
    model: SpeechCommandsGruClassifier,
    ...
    *,
    ...
    model.gru.calibrating = False
    model.gru.finalize_calibration()
    model.gru.use_quantization = True
    calibration = {
        "batch_count": config.calibration_batches,
        "method": model.gru.calibration_method,
    }
...
def _quantization_error(
    ...
    labels = []
    with torch.no_grad():
        for features, batch_labels in _batches(*data, batch_size):
            features = features.to(device, non_blocking=True)
            model.gru.use_quantization = True
            quantized_logits.append(model(features))
            model.gru.use_quantization = False
    ...
    return {
        ...
        "normalized_mae": difference.abs().mean().item()
        / max(reference_mean_abs, 1.0e-12),
        "normalized_rmse": difference.square().mean().sqrt().item()
        / max(reference_rms, 1.0e-12),
        "absolute_error_p50": torch.quantile(absolute_error, 0.50).item(),
        "absolute_error_p95": torch.quantile(absolute_error, 0.95).item(),
        "absolute_error_p99": torch.quantile(absolute_error, 0.99).item(),
        "absolute_error_max": absolute_error.max().item(),
        "prediction_agreement": (
            quantized.argmax(dim=1) == reference.argmax(dim=1)
        ).double().mean().item(),
        "quantized_accuracy": (
            quantized.argmax(dim=1) == targets
        ).double().mean().item(),
        "float_path_accuracy": (
            reference.argmax(dim=1) == targets
        ).double().mean().item(),
        "reference_min": reference.min().item(),
        "reference_max": reference.max().item(),
    }
...
def _train(
    ...
    for epoch in range(config.epochs):
        ...
        if (
            ...
            refreshed = _calibrate_quant_gru(
                model,
...
def run_training_comparison(config: ExperimentConfig) -> dict:
    """Train matched torch-GRU and QuantGRU-QAT keyword spotters."""
    config.validate()
    config = ExperimentConfig(
        **{**asdict(config), "dataset_root": Path(config.dataset_root)}
    )
    if not torch.cuda.is_available():
        raise RuntimeError("QuantGRU real-network training requires CUDA")
    ...
    baseline = SpeechCommandsGruClassifier(
        config.hidden_size,
        len(config.labels),
        use_quant_gru=False,
        device=device,
    )
    native_float = SpeechCommandsGruClassifier(
        config.hidden_size,
        len(config.labels),
        use_quant_gru=True,
        device=device,
    ...
    quantized_variants = {}
    initial_differences = {
        "quant_gru_float": _copy_shared_initial_state(baseline, native_float)
    }
    calibrations = {}
    for bitwidth in config.quant_bitwidths:
        name = f"quant_gru_qat_{bitwidth}bit"
        quantized = SpeechCommandsGruClassifier(
            config.hidden_size,
            len(config.labels),
            use_quant_gru=True,
            device=device,
        ...
        calibrations[name] = _calibrate_quant_gru(
            quantized, feature_sets["training"], config, device, bitwidth
        )
        quantized_variants[name] = quantized

    baseline_result = _train(
        "torch_gru", baseline, feature_sets, config, device
    )
    native_float_result = _train(
        "quant_gru_float", native_float, feature_sets, config, device
    )
    quantized_results = {
        name: _train(
            name,
    ...
    return {
        ...
        "model": {
            "source": (
                "google-research/google-research@4700efb9afa54286b0e04473ba80a13e8461e25f/"
                "kws_streaming/models/gru.py"
            ),
            "topology": "20-coefficient MFCC -> GRU -> dropout(0) -> dense",
        ...
        "replacement": {
            "changed_module": "gru",
            "baseline": "torch.nn.GRU",
            "candidate": "quant_gru.QuantGRU",
            "initial_shared_state_max_abs_diff": initial_differences,
        },
        "quantization": {
            "mode": "qat",
            "scale_encoding": "affine_m_plus_shift",
            "quantized_value_storage": "float32_integer_carrier",
        ...
        "training": {
            "torch_gru": baseline_result,
            "quant_gru_float": native_float_result,
            **quantized_results,
        },
    }
...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cache-root",
        type=Path,
        default=Path.home() / ".cache" / "quant-gru" / "speech_commands_v0.02",
    )
